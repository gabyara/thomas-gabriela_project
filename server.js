...
app.get('/api/getFile/:name/:folder', function(req, res){
  console.log("Folderrrr",req.params.folder)
  res.sendFile(basepath + '/'+req.params.folder+'/'+req.params.name);
}); 
...
app.post('/api/deleteFile/:folder', async function(req,res){
  ...
  for(var i=0;i<req.body.length; i++){
    try {
      await fs.unlink(basepath + '/'+req.params.folder+'/'+req.body[i] ,function(err, result){
...
app.post('/api/file1/:folder', (req, res) => {
  console.log("folder",req.params.folder);
  console.log("name",req.body.file.name);
  try {
    fs.writeFile("./"+req.params.folder+"/"+req.body.file.name, req.body.file.content ,function(err, result){
      if(err) console.log('error', err);
    })
  } catch (err) {
    console.error("error aquiiii", err)
...
var fs = require('fs');
console.log("toFolder",req.params.name)
var dir = './'+req.params.name;

if (!fs.existsSync(dir)){
    fs.mkdirSync(dir); 
    // fs.copyFile('./kYS1cBJ6oqukp0KTeDScD7XHvh5WJF36nDk=', './'+req.params.name+'/kYS1cBJ6oqukp0KTeDScD7XHvh5WJF36nDk=', (err) => {
    //   if (err) throw err;
